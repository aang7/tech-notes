...
** Updating DB Data after a migration

Instead of updating user data inside a migration, what we can do is build an sql script with the statements wrapped inside a transaction.
Doing it so, we can prevent having unsync or corrupted data.
...
#+begin_src sql :eval no :exports code 
